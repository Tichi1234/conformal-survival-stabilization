suppressPackageStartupMessages({library(dplyr); library(tidyr); library(readr); library(purrr); library(tibble); library(ggplot2); library(stringr); library(forcats)})  # Load the tidyverse package without displaying messages
