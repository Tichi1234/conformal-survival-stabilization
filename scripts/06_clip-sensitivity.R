...
##
## Sensitivity of the clipped methods to the clipping percentile q.
## For each q, the clipping threshold is estimated from the TRAINING fold
## and then held fixed during calibration. Data, splits, seeds and nuisance
## fits are held fixed across q so differences are attributable to the
## clipping level. q = 1.00 sets c = Inf and gives the unclipped Hajek baseline.
##
## Output:
...
one_rep <- function(n, setting, mech, cens, seed) {
  ...
  ## Select clipping threshold from TRAINING fold only
  cclip <- if (q >= 1) {
    Inf
  } else {
    select_clip_percentile(
      nu = nu,
      train = train,
      q = q
    )$cclip
  ...
  ## Hold the training-selected threshold fixed during calibration
